...
describe("FundMe", function () {
  ...
  const sendValue = ethers.utils.parseEther("1"); // Ether to Wei

  beforeEach(async function () {
    deployer = (await getNamedAccounts()).deployer; // Wallet address of the deployer
  ...
  describe("constructor", function () {
    ...
    it("sets the aggregator address correctly", async function () {
      ...
      assert.equal(mockV3AggregatorAddress.toString(), response.toString());
    });
  });
  ...
  describe("fund", async function () {
    ...
    it("added the funder address in funders", async function () {
      await fundMe.fund({ value: sendValue });
  ...
  describe("withdraw", async function () {
    ...
    it("withdraw funds from multiple funders", async function () {
      ...
      // addressToAmoundFunded amounts should be 0 after widthrawal
      for (let i = 1; i < 6; i++) {
        assert.equal(
